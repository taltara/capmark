 ...
 *
 * - imports of Node builtins. Injecting the `fs` service is not the only way to
 *   touch a disk, and an early version of this reader assumed it was. Ten of
 *   the sixteen published plugins audited use `node:fs` directly while
 *   injecting no `fs` service at all, and one opens raw sockets through
 *   `node:net`. A draft that read only the service graph would have called
 *   those plugins capability-free — a false security claim, generated by the
 *   tool that exists to prevent them.
 *
...
export interface Inference {
  ...
  /** Node builtins it imports, which bypass the service graph entirely. */
...
/**
 * Node builtins that reach past the harness entirely.
 *
 * A plugin importing these needs no service and asks no permission — the
 * capability is real whether or not anything in Cordis knows about it.
 */
const BUILTIN_CAPABILITY: Readonly<Record<string, string>> = {
  fs: 'fs:read',
  'fs/promises': 'fs:read',
  child_process: 'proc:spawn',
  net: 'net:fetch',
  dns: 'net:fetch',
  http: 'net:fetch',
  https: 'net:fetch',
...
const BUILTIN_IMPORT = /["']node:([a-z_]+(?:\/[a-z]+)?)["']/g
...
/** Node builtins a source file imports, deduplicated. */
export function readBuiltins(source: string): string[] {
  const found = new Set<string>()
  for (const match of source.matchAll(BUILTIN_IMPORT)) {
    const name = match[1]
...
function jsFiles(dir: string, depth = 0): string[] {
  ...
  for (const entry of entries) {
    ...
    // `.mjs` and `.cjs` count: dsh-mask ships `index.mjs` as its main, and a
    // reader that only looked at `.js` skipped the package altogether while
    // reporting a confident empty result.
    else if (/\.(js|mjs|cjs)$/.test(entry) && !entry.endsWith('.map.js'))
      out.push(full)
  }
  return out
}
...
export function infer(packageDir: string): Inference {
  ...
  for (const file of jsFiles(packageDir)) {
    ...
    for (const name of readBuiltins(source)) builtins.add(name)
  }
  ...
  for (const builtin of [...builtins].sort()) {
    const capability = BUILTIN_CAPABILITY[builtin]
    if (capability === undefined || seen.has(capability)) continue
    seen.add(capability)
    proposed.push({ capability, evidence: `imports \`node:${builtin}\`` })
  }
  proposed.sort((a, b) => a.capability.localeCompare(b.capability))
